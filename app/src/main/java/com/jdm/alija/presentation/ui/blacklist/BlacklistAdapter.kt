...
class BlacklistAdapter(
    ...
    inner class ViewHolder(
        val binding: ItemBlacklistBinding,
        onItemSwitchClick: (BlackContact, Int) -> Unit
    ) : RecyclerView.ViewHolder(
        binding.root
    ) {
        init {
            binding.ivItemBlacklistDelete.setOnClickListener {
        ...
        fun bindView(item: BlackContact) {
            with(binding) {
                tvItemBlacklistName.text = item.name
                tvItemBlacklistMobile.text = item.mobile
            }
        }
    }
}
